...
class Graph:
    ...
    Task function parameters become CLI flags automatically.  Use
    ``Annotated[T, Param(...)]`` for fine control over help text,
    short flags, and global vs. local namespace.
    ...
    def _build_kwargs(
        ...
        """Build function kwargs from run parameters and task input.

        Resolution order for each parameter:

        1. *task_input* (array expansion payload, e.g. ``item``)
        2. Task-local parameters stored under
           ``run_parameters["__task_params__"][task_name]``
        3. Global *run_parameters*
        ...
        spec : TaskSpec
            Task specification.
        run_parameters : Dict[str, Any]
            Stored run parameters (includes ``__task_params__`` for locals).
        task_input : Dict[str, Any]
            Per-instance task input payload.
        ...
        """
        # Local parameters for this specific task
        task_locals: Dict[str, Any] = (
            run_parameters
            .get("__task_params__", {})
            .get(spec.name, {})
        )
        ...
        for param_name in spec.signature.parameters:
            ...
            elif param_name in task_locals:
                kwargs[param_name] = task_locals[param_name]
